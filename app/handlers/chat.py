...
TOOLS_HINT = (
    ...
    "или ПОЛНЫЙ новый план (set_plan). Если клиент говорит, что съел что-то "
    "(«съел…», «на обед было…», «выпил американо», «тот сырок/котлета что ел раньше») — "
    "используй log_meal, передав описание как есть; если он ссылается на ранее съеденное "
    "блюдо — так и укажи в description (система сопоставит с историей). Изменения "
    "применяются ТОЛЬКО после подтверждения кнопкой.\n"

...
def deny_message(reason: str | None, action: str, activated: bool = True) -> str:
    """Дружелюбное сообщение при отказе. activated=False → это пробный лимит (не суточный)."""
    if reason == "fuse":
        return (
    ...
    if not activated:
        # Пробный лимит без TTL — «обнулится завтра» тут неверно; ведём к активации.
        return (
            f"Пробные {what} на этом аккаунте закончились 🙂\n"
            "Заверши настройку профиля и первую тренировку — и откроется ежедневный лимит. "
            "Тренировки, дневник и статистика доступны без ограничений."
        )
    return (
        f"На сегодня лимит {what} исчерпан 🙂 Обнулится завтра.\n"
        "Тренировки, дневник и статистика работают без ограничений."
    )

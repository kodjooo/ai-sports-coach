...
def feedback_prompt(facts: str, memory: str, today: str) -> str:
    ...
    return (
        ...
        f"СЕГОДНЯ:\n{today}\n\n"
        "Задача: дай короткий поддерживающий фидбек (2–4 предложения): что получилось, "
        "на что обратить внимание по технике/самочувствию. ВАЖНО: НЕ назначай подходы и "
        "повторы на следующий раз — нагрузку система пересчитывает сама по факту выполнения "
        "и покажет отдельным сообщением. Не перечисляй программу списком."
    )

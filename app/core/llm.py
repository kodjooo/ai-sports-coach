...
_FOOD_SCHEMA_HINT = (
    ...
    "ВАЖНО: если на фото ЭТИКЕТКА/упаковка с указанной пищевой ценностью (ккал и БЖУ на "
    "100 г или на порцию) — бери ЭТИ значения как есть (пересчитав под съеденную порцию/граммы), "
    "НЕ угадывай; в таких позициях поставь \"source\": \"label\" (query можно оставить пустым). "
